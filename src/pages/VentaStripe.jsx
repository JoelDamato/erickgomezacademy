...
function VentaStripe() {
  ...
  const generarLinkDePago = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/stripe-link`, {
        method: "POST",
      ...
      if (response.ok && data.link) {
        window.location.href = data.link; // redirige a Stripe
      } else {
        throw new Error(data.error || "Error desconocido");
      }
    } catch (err) {
      console.error("❌ Error al generar el link de pago:", err);
      setError("Hubo un problema generando el link de pago. Intentalo de nuevo.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center text-white px-4 relative bg-[url('https://i.ibb.co/6JRvGQ0M/Fondosinega2.png')] bg-cover bg-center">
      <div className="bg-black/80 backdrop-blur-md p-8 rounded-xl max-w-lg w-full shadow-lg">
        <div className='flex justify-center'>
          <img src="https://i.ibb.co/bR6KXLbb/Master-Fade-3-0.png" className="w-40 h-40 my-3" alt="Master Fade" />
        </div>
        ...
        <p className="text-xs text-center mt-5 text-zinc-300">
          Al hacer clic en <u>“Ir a pagar”</u>, vas a ser dirigido a Stripe.
          <br />
          ...
          <br />
          <u>Solo tenés que esperar 5 segundos</u> luego de pagar dentro de Stripe.
        </p>
      </div>
    </div>
  );
