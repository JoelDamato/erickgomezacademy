...
function Dashboard() {
  ...
  const bienvenidaBanner = {
    ...
    courseDescription:
      "Bienvenido/a. Este no es solo un curso: es el inicio de tu transformación como barbero. Desde hoy sos parte de una comunidad de +10,000 barberos que crecen y se apoyan. No solo aprenderás técnica, sino a dejar tu marca en la industria. Este es tu momento. Este es tu lugar. Dale play… y empecemos.",
  };

  // Reordeno: primero bienvenida, luego Master Fade si existe, luego el resto
  const masterFade = coursesWithBanner.find((c) =>
    c.courseTitle?.toLowerCase().includes("master fade")
  );
          ...
          <div className="w-full max-w-6xl mx-auto px-0 md:px-4 pt-0 md:pt-8">
            ...
            >
              {bannersToShow.map((curso, index) => (
                <div
                  key={index}
                  className="w-full flex-shrink-0 snap-center px-0 md:px-2"
                  style={{ minWidth: "100%", maxWidth: "100%" }}
                >
                  <div
                    className={`relative text-white ${
                      index === activeIndex ? "opacity-100" : "opacity-90"
                    } h-[100%] md:h-3/4 md:mt-10 transition-opacity duration-500 md:border border-gray-700 rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row`}
                  >
                    {/* Imagen del banner */}
                    <img
                      src={curso.banner}
        ...
        <div className="w-full px-4 mt-10 ">
          {user?.cursos?.length > 0 ? (
            ...
            <div className="text-center mt-10">
              <h1 className="text-3xl mb-6 font-bold text-white">
                Aún no tienes ningún entrenamiento desbloqueado!
              </h1>
              <button
                onClick={() =>
                  window.open(
                    `https://wa.me/${phoneNumber}?text=${encodeURIComponent(
                      "Hola, quiero empezar mi entrenamiento."
                    )}`,
                    "_blank"
                  )
                }
          ...
          <div className="flex justify-center">
            <div className="flex overflow-x-auto gap-4 no-scrollbar">
              {courses
                .filter((c) => hasCourse(c.courseTitle))
                .map((course, index) => (
                  <div
                    key={index}
                    className="min-w-[160px] max-w-[160px] flex-shrink-0 rounded-md shadow-md p-2"
                  >
                    <a
                      onClick={() =>
                        navigate(
